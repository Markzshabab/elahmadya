const app = {
    ...
    async initMedia(video, audio) {
        ...
        } catch (err) { alert('يرجى إعطاء الصلاحية للكاميرا أو الميكروفون للتسجيل.'); }
    },
    ...
    async submitSurvey() {
        ...
        try {
            const res = await fetch(`${this.apiUrl}/submit`, { method: 'POST', body: formData });
            ...
            alert(e.message.includes('حظر') ? "عذراً، تم حظر عنوان IP الخاص بك من المشاركة." : "حدث خطأ أثناء إرسال الاستبيان، يرجى المحاولة مرة أخرى.");
        }
    },
