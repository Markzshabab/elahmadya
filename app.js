const app = {
    ...
    async checkServerStatus(fingerprint) {
        try {
            const res = await fetch(`${this.apiUrl}/check-status`, {
                method: 'POST',
            ...
            if (data.has_voted || data.banned) {
                // الخادم يؤكد أن هذا الجهاز/IP قد صوت أو محظور
                this.markAsVoted();
                this.showAlreadyVotedScreen(data.banned);
            }
        } catch (error) {
